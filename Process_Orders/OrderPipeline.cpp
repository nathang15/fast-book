...
void OrderPipeline::processOrdersFromFile(const std::string& filename) {
	...
	std::ofstream csvFile("order_processing_times.csv", std::ios::trunc);
	if (!csvFile.is_open()) {
...
void OrderPipeline::processAddLimitOrder(std::istringstream& iss) {
	int orderId, shares, price;
	bool buyOrSell;
	iss >> orderId >> buyOrSell >> shares >> price;
	book->addLimitOrder(orderId, buyOrSell, shares, price);
}
...
void OrderPipeline::processModifyLimitOrder(std::istringstream& iss) {
	int orderId, shares, price;
	iss >> orderId >> shares >> price;
	book->modifyLimitOrder(orderId, shares, price);
}
...
void OrderPipeline::processModifyStopLimitOrder(std::istringstream& iss) {
	int orderId, shares, stopPrice, limitPrice;
	iss >> orderId >> shares >> stopPrice >> limitPrice;
	book->modifyStopLimitOrder(orderId, shares, stopPrice, limitPrice);
}
